...
if [[ "$SOLO_MODE" != "true" ]]; then
    ...
    # OVERRIDE: Force the script to use the first IP from --nodes
    LOCAL_IP="${ALL_NODES[0]}"
fi
...
start_cluster() {
    ...
    for worker in "${PEER_NODES[@]}"; do
        ...
        local docker_run_cmd="docker run $docker_caps_args $docker_resource_args -e VLLM_HOST_IP=$worker $docker_args_common"
